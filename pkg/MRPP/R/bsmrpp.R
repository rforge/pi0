...
#    lastperm=perm1=permutedTrt[,1]; lastcperm=cperm1=cperm.mat[,1]
    ...
    for(b.i in seq_along(bseqs)) {
        if(verbose && isTRUE(b.i%%verbose==0L)) cat("iteration:",b.i-1L," out of",Bperm,"\t\t\r")
...
#        permutedTrt[,b]=perm1;  cperm.mat[,b]=cperm1
        ...
        selected.pvals[b.i]={tmp=tail(mrppBVS(y=y,permutedTrt=permutedTrt, verbose=FALSE, niter=niter, 
    ...
    bsfit$statistic=selected.pvals[1]
    ...
    bsfit$parameter=unlist(attr(fit0, 'parameter'))
    bsfit$data.name=paste("Response data", deparse(substitute(x)), 'and permuted treatment', deparse(substitute(permutedTrt)))
    class(bsfit)='htest'
    bsfit
}
